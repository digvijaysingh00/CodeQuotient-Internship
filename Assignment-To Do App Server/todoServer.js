...
function readDatabase(callback) {
    fs.readFile("./CodeQuotient-Internship/Assignment-To Do App Server/myDatabase.txt", "utf-8", function (err, data) {
        if (err) {
            callback(err);
            return;
        }
...
function saveDatabase(todo, callback) {
    readDatabase(function (err, data) {
        ...
        data.push(todo);
        fs.writeFile("./CodeQuotient-Internship/Assignment-To Do App Server/myDatabase.txt", JSON.stringify(data), function (err) {
            if (err) {
                callback(err);
                return;
            }
